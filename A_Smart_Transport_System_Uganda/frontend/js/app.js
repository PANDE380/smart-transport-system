...
let liveLocationsStream = null;
let liveLocationsStreamConnected = false;
...
const PAYMENT_METHOD_META = {
    ...
    CARD: {
        ...
        helper: 'Enter your 16-digit card number. A secure gateway pop-up will appear.'
    },
    CASH: {
        label: 'Cash via STS Agent',
        accountLabel: 'Agent ID or Kiosk Number',
        placeholder: 'STS-AG-1234',
        helper: 'Provide the official 8-to-10 character agent ID handling your physical cash.'
    }
};
...
async function boot() {
    ...
    if (savedUser) {
        try {
            ...
            // Clear stale session data so the user doesn't get stuck
            localStorage.removeItem('user');
            currentUser = null;
            setGuestUI();
            showPg('home');
            // Only show a toast if it was a real server error, not just a missing user
            if (e.message && !e.message.includes('404')) {
                setTimeout(() => showT('⚠️', 'Your previous session has expired. Please sign in again.', 'var(--orange)'), 500);
            }
        }
    } else {
        setGuestUI();
...
async function doLogin() {
    ...
    await withLoading(btn, async () => {
        ...
        try {
            ...
            showLoginErr('Unable to reach the server. Please try again.');
        ...
        if (!response.ok) {
            const errorMessage = result.error || result.message || `Login failed (${response.status})`;
            showLoginErr(errorMessage);
            throw new Error(errorMessage);
        }

        if (!result || typeof result !== 'object') {
            showLoginErr('Unexpected server response during login.');
        ...
        if (!result.user) {
            showLoginErr('Login succeeded but no user data was returned.');
...
function setLoggedInUI(user) {
    ...
    buildNav(nav);
    const admLink = document.getElementById('nav-admin-link');
    if (admLink) admLink.style.display = user.role === 'admin' ? 'inline-block' : 'none';
    const bui = document.getElementById('booking-ui');
    const gp = document.getElementById('guest-prompt');
    const sw = document.getElementById('sc-wrap');
...
function buildNav(cfg) {
    // Dynamic array nav generation is disabled in favor of static structural UI in index.html.
    // Structural layout handles dropdowns natively, while syncActiveNav still handles role gating.
    syncActiveNav();
}
...
function showPg(name, btn) {
    ...
    if (PAGE_RESTRICTIONS[name] && !PAGE_RESTRICTIONS[name].includes(role)) {
        ...
        // If they are on a safe page, stay there, otherwise go home
...
async function initLiveTracking() {
    if (liveLocationsStream) return;

    liveLocationsStream = new EventSource(`${API_BASE_URL}/vehicles/live-locations/stream`);
    
    liveLocationsStream.addEventListener('live_locations', (event) => {
        liveLocationsStreamConnected = true;
        try {
            ...
            console.warn('Live tracking parse skipped', e);
        }
    });

    liveLocationsStream.addEventListener('heartbeat', () => {
        liveLocationsStreamConnected = true;
    });

    liveLocationsStream.onopen = () => {
        liveLocationsStreamConnected = true;
    ...
    liveLocationsStream.onerror = () => {
        liveLocationsStreamConnected = false;
        if (liveLocationsStream) {
            liveLocationsStream.close();
            liveLocationsStream = null;
        }
        // Fallback to initial SSE connection loop
        setTimeout(initLiveTracking, 3000);
    };
}
